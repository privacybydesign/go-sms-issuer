...
// RedisSeenStore is a pow.SeenStore backed by Redis, so single-use tracking
...
func NewRedisSeenStore(client *redis.Client, namespace string) *RedisSeenStore {
	return &RedisSeenStore{client: client, namespace: fmt.Sprintf("%s:pow-seen", namespace)}
}
